...
using ChatListControl._ChatListBox;

namespace ChatListControl
{
    public partial class ChatListBox : Control
    {
        public ChatListBox() {
            ...
            this.SetStyle(ControlStyles.SupportsTransparentBackColor, true);
             this.MouseDown+=new MouseEventHandler(ChatListBox_MouseDown);
        }
    
        public void ChatListBox_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        #region Properties
        private bool isDrawOpen=true;
        [Browsable(true)]
        [DefaultValue(true)]
        [Description("指示是否需要绘制箭头")]
        public bool IsDrawOpen
        {
            get { return isDrawOpen; }
            set
            {
                if (isDrawOpen == value) return;
                isDrawOpen = value;
                this.Invalidate();
            }
        }
        ...
        [Browsable(false)]
        public List<ChatListSubItem> SubItemItems
        {
            get
            {
                List<ChatListSubItem> listSubItem = new List<ChatListSubItem>();
                foreach (var item in Items.m_arrItem)
                {
                    if (item==null || item.SubItems.m_arrSubItems == null) continue;
                    foreach (var subitem in item.SubItems.m_arrSubItems)
                    {
        ...
        public void RemoveSubItem(ChatListSubItem subItem)
        {
            bool isFind = false;
            foreach (ChatListItem item in Items.m_arrItem)
            {
                if (item == null || item.SubItems.m_arrSubItems == null) continue;
                foreach (var subitem in item.SubItems.m_arrSubItems)
                {
        ...
        /// <summary>
        /// 排序方式 1是检索式，2 为好友式，3为最近联系人式
        /// </summary>
        [Description("项排序方式,1是检索式，2 为好友式，3为最近联系人式")]
        public int SortType
        {
            get { return sortType; }
            set { sortType = value; }
        ...
        //bool isStartListenTwinkle = true;
        //[Description("是否启动头像闪烁")]
        //public bool IsStartListenTwinkle
        //{
        //    get { return isStartListenTwinkle; }
        //    set { isStartListenTwinkle = value; }
        //}
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            #region
            //if (Items.m_arrItem != null && SubItemItems.Count > 0)
            //{
            //    TipMsg = "";
            //}
            ...
            //{
            //    g.DrawString(TipMsg, new Font("微软雅黑", 12, FontStyle.Bold), new SolidBrush(Color.Gold), new PointF(30, 100));
        ...
        public ChatListSubItem GetChatListSubItemWithMouseLocation()
        {
            return m_mouseOnSubItem;
        }
       public ChatListItem GetChatListItemWithMouseLocation()
        {
            return m_mouseOnItem;
       }
       protected override void OnMouseMove(MouseEventArgs e)
       {
                ...
                if (item.Bounds.Contains(m_ptMousePos)) {
                    ...
                    if (item.IsOpen) {
                        ...
                        if (new Rectangle(0, item.Bounds.Top, this.Width, 20).Contains(m_ptMousePos)) {
                            selectSubItem = null;
                            if (mouseEvent.Button == System.Windows.Forms.MouseButtons.Right) return;
        ...
        protected virtual void DrawItem(Graphics g, ChatListItem item, Rectangle rectItem, SolidBrush sb) {
         ...
         //  if (item.Equals(m_mouseOnItem))           //根据列表项现在的状态绘制相应的背景色
               if (item.Bounds.Contains(this.PointToClient(MousePosition)) )         //根据列表项现在的状态绘制相应的背景色
            ...
            string strItem = (!isDrawOpen?"":"\t") + item.Text;
            sb.Color = this.ForeColor;
            sf.Alignment = StringAlignment.Near;
            g.DrawString(strItem, this.Font, sb, rectItem, sf);
            sf.Alignment = StringAlignment.Near;
            Size szFont = TextRenderer.MeasureText(strItem, this.Font);
            g.DrawString("[" + item.SubItems.GetOnLineNumber() + "/" + item.SubItems.Count + "]", this.Font, sb,
                new Rectangle(rectItem.X + szFont.Width +  25, rectItem.Y, rectItem.Width - 15, rectItem.Height), sf);
        }
        /// <summary>
        /// 绘制列表子项
        /// </summary>
        ...
        protected virtual void DrawSubItem(Graphics g, ChatListSubItem subItem, ref Rectangle rectSubItem, SolidBrush sb) {
            if (subItem.Equals(selectSubItem) && isDrawOpen)
            {        //判断改子项是否被选中
        ...
        protected virtual void DrawHeadImage(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem) {
            ...
            if (subItem.HeadImage == null)                 //如果头像位空 用默认资源给定的头像
                subItem.HeadImage = global::ChatListControl.Properties.Resources._null;
            if (subItem.Status == ChatListSubItem.UserStatus.OffLine)
                ...
                //    g.DrawImage(global::ChatListControl.Properties.Resources.QMe, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == ChatListSubItem.UserStatus.Away)
                //    g.DrawImage(global::ChatListControl.Properties.Resources.Away, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == ChatListSubItem.UserStatus.Busy)
                //    g.DrawImage(global::ChatListControl.Properties.Resources.Busy, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == ChatListSubItem.UserStatus.DontDisturb)
                //    g.DrawImage(global::ChatListControl.Properties.Resources.Dont_Disturb, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                //if (subItem.Status == ChatListSubItem.UserStatus.Invisible)
                //    g.DrawImage(global::ChatListControl.Properties.Resources.Invisible, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
        ...
        protected virtual void DrawSmallSubItem(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem) {
            rectSubItem.Height = (int)ChatListItemIcon.Small;   
            //重新赋值一个高度
            ...
            string strDraw = subItem.DisplayName;
                string ltime = subItem.LastReceiveMsgTime == null || (subItem.LastReceiveMsgTime<new DateTime(2014,11,30)) ? string.Empty : "[" + getTimeDescription((DateTime.Now - subItem.LastReceiveMsgTime)) + "]";
                ...
                g.DrawString("\t" + subItem.PersonalMsg + ltime, this.Font, Brushes.Gray, rectSubItem, sf);
            
        }
        string getTimeDescription(TimeSpan tSpan)
        {
            if (tSpan.Days>0  )
                return tSpan.Days+ "天前";
            else if (tSpan.Hours>0 )
               return tSpan.Hours+ "小时前";
            else if (tSpan.Minutes>0)
                return tSpan.Minutes + "分钟前";
            else
        ...
        public ChatListItem[] GetItemsById(int userId)
        {
            List<ChatListItem> subItems = new List<ChatListItem>();
            for (int i = 0, lenI = this.items.Count; i < lenI; i++)
            {
        ...
        public ChatListSubItem[] GetSubItemsByText(  string searchText)
        {
            List<ChatListSubItem> subItems = new List<ChatListSubItem>();
            for (int i = 0, lenI = this.items.Count; i < lenI; i++)
            {
                if (items[i].SubItems.m_arrSubItems == null) continue;
                var result = items[i].SubItems.m_arrSubItems.Where(b => b != null).Where(a => (a.DisplayName!=null&&a.DisplayName.Contains(searchText)) ||
                    a.NicName.Contains(searchText));
        ...
        public class ChatListItemCollection : IList, ICollection, IEnumerable
        {
            ...
            //public ChatListItem this[string ItemText]
            //{
            //    get
            //    {
            //        //不知道为什么总有null值
            //        var result = m_arrItem.Where(a => a != null && a.Text == ItemText);
            //        if (result.Count() > 1)
            //        {
            //            throw new Exception("存在多个[" + ItemText + "]分组");
            //        }
